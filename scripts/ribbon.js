...
function init() {
    var colors;
    var redColors = [0xff0000, 0xb20000, 0x7f0000, 0xffffff];
    var orangeColors = [0xffd700, 0xffa500, 0xff8c00, 0xffffff];
    var yellowColors = [0xffff66, 0xffff00, 0x999900, 0xffffff];
    var greenColors = [0x00ff00, 0x00b200, 0x006600, 0xffffff];
    var blueColors = [ 0x0900ff, 0x0078ff, 0x00f9ff, 0xffffff];
...
function render() {
    ...
    for (var j = 0; j < points.length; j++){
        ...
        // var rotationMatrix = new THREE.Matrix4().makeRotationZ( Math.cos(j)/50 );
        // camera.up.applyMatrix4(rotationMatrix);
        // point.position.z = (-(timeFrequencyData[j])/30);
        // }
        // console.log(j);
    ...
    var rotationMatrix = new THREE.Matrix4().makeRotationZ( Math.tan(1)/100 );
    camera.up.applyMatrix4(rotationMatrix);
